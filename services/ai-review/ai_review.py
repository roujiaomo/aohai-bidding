...
def review_stats(c: sqlite3.Connection) -> dict:
    """Return counters for the two-list review page using one denominator."""
    active = [r for r in rows(c.execute("SELECT ai_status,deadline_at,content FROM reviews"))
              if r['ai_status'] != 'expired' and not tender_has_passed_deadline(r)]
    approved = sum(r['ai_status'] == 'approved' for r in active)
    approved_manual = sum(r['ai_status'] == 'approved_manual' for r in active)
    exclude = c.execute("SELECT COUNT(*) FROM reviews WHERE ai_status='exclude'").fetchone()[0]
    return {
        "total": approved + approved_manual + exclude,
        "pending": sum(r['ai_status'] == 'pending' for r in active),
        "approved": approved,
        "approved_manual": approved_manual,
        "manual_review": sum(r['ai_status'] == 'manual_review' for r in active),
        "rejected": sum(r['ai_status'] in ('rejected', 'rejected_manual') for r in active),
        "exclude": exclude,
    }
...
async function loadRows(page=1){loading(true);try{const [s,r]=await Promise.all([A('api/stats'),A(`api/records?list=${state.list}&page=${page}&page_size=${state.pageSize}`)]);state.stats=s;state.page=r.page;state.total=r.total;state.pageSize=r.page_size;hint.textContent='AI 排除仅供复核；AI 审批通过的有效公告与实时商机一一对应。';stats.innerHTML=`<div class=stat><div class=num>${s.total||0}</div><div class=label>通过与排除合计</div></div><div class=stat><div class=num>${(s.approved||0)+(s.approved_manual||0)}</div><div class=label>可进入实时商机</div></div>`;renderTabs();renderRows(r.rows||[])}catch(e){records.innerHTML=`<div class="panel empty">加载失败：${esc(e.message)}</div>`}finally{loading(false)}}
